...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    const lics = await db.query('SELECT l.*, p.label as plan_label FROM licenses l LEFT JOIN plans p ON l.plan_id=p.id WHERE l.user_id=$1 ORDER BY l.created_at DESC', [user.id]);
    const token = jwt.sign({ userId: user.id, email: user.email, role: user.role }, JWT_SECRET, { expiresIn: '30d' });
    res.json({ success: true, token, user: { id:user.id, name:user.name, email:user.email, role:user.role }, licenses: lics.rows });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
router.get('/me', authMiddleware, async (req, res) => {
  try {
    const lics = await db.query('SELECT l.*, p.label as plan_label FROM licenses l LEFT JOIN plans p ON l.plan_id=p.id WHERE l.user_id=$1 ORDER BY l.created_at DESC', [req.user.id]);
    res.json({ success: true, user: { id:req.user.id, name:req.user.name, email:req.user.email, role:req.user.role }, licenses: lics.rows });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
