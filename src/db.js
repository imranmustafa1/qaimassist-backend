...
async function initDB() {
  ...
  // Safe column migrations — fix ANY missing columns
  const migrations = [
  ...
  for (const sql of migrations) {
    await db.query(sql).catch(e => console.log('Migration skipped:', e.message));
  }

  // Fix licenses that have plan_id but no plan_name
  await db.query(`
    UPDATE licenses SET plan_name='monthly' WHERE plan_name IS NULL OR plan_name='';
  `).catch(() => {});
  ...
  // Superadmin — always refresh password hash
  ...
  console.log('✅ Database ready — admin password reset to:', adminPass);
}
