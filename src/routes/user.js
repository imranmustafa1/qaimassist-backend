...
router.get('/dashboard', authMiddleware, async (req, res) => {
  try {
    const lics = await db.query('SELECT l.*, p.label as plan_label, p.features FROM licenses l LEFT JOIN plans p ON l.plan_id=p.id WHERE l.user_id=$1 ORDER BY l.created_at DESC', [req.user.id]);
