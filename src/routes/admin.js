...
router.get('/stats', async (req, res) => {
  try {
    const [users, licenses, contacts] = await Promise.all([
      ...
      db.query("SELECT COUNT(*) as total, COUNT(*) FILTER(WHERE status='active') as active, COUNT(*) FILTER(WHERE status='expired') as expired, COUNT(*) FILTER(WHERE plan='monthly') as monthly, COUNT(*) FILTER(WHERE plan='yearly') as yearly, COUNT(*) FILTER(WHERE plan='lifetime') as lifetime, COUNT(*) FILTER(WHERE plan='trial') as trial FROM licenses"),
...
router.get('/users', async (req, res) => {
  ...
  try {
    let q = `SELECT u.*, COUNT(l.id) as license_count, MAX(l.plan) as latest_plan FROM users u LEFT JOIN licenses l ON l.user_id=u.id WHERE u.role='user'`;
    const params = [];
...
router.get('/licenses', async (req, res) => {
  ...
  try {
    let q = `SELECT l.*, u.name as user_name, u.email as user_email, p.label as plan_label, p.price_usd, p.price_pkr, p.duration_days
             FROM licenses l LEFT JOIN users u ON l.user_id=u.id LEFT JOIN plans p ON l.plan=p.name WHERE 1=1`;
    const params = [];
    ...
    const r = await db.query(q, params);
    const rows = r.rows.map(l => ({ ...l, plan_name: l.plan, email: l.email || l.user_email }));
...
router.post('/licenses/:key/extend', async (req, res) => {
  try {
    ...
    await db.query(`UPDATE licenses SET expires_at=COALESCE(expires_at,NOW()) + ($1::text||' days')::interval, status='active' WHERE key=$2`, [days.toString(), req.params.key]);
    res.json({ success: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/licenses/:key/upgrade', async (req, res) => {
  ...
  try {
    ...
    const p = planInfo.rows[0];
    const expiresAt = p.duration_days ? new Date(Date.now() + p.duration_days * 86400000) : null;
    await db.query(`UPDATE licenses SET plan=$1, expires_at=$2, status='active' WHERE key=$3`, [p.name, expiresAt, req.params.key]);
    res.json({ success: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
